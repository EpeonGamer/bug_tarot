...
5. Pass reference to game handler to cards, so they can trigger methods to queue and activate certain states in it (as well as query select limits)
6. Statemachine controlling whether drawing etc..
		1. Communicate state and limits
7. extend ActionElement to incorprate element and bug specific features (eg. cost/trigger)
## Board states
8. Cards are all not interactable.
9. When a card is placed/reactivated, relevant cards must be highlighted until a mi or max limit is reached.
10. After an "accept" input, the action(s) must be carried out.
...
## Dev Notes
20. Nerf centipede or buff grub since similar effects
	1. Roach lack of limitation is also very powerful, as is snail
21. Unify draw and discard pile (since players don't want to discard other's cards given win condition)?
	1. Change win condition for end-game rush
22. Specify whether hand, placed, or discard pile affected.
23. Simplify player range (scope) to self or other
	1. You can only effect hand, placed, OR deck/discard.
24. Re-evaluate deckbuilding as all same bug/psyche/element means quick win
	1. Maybe limit to 3 cards, whole deck build for special game mode, and/or shuffle all player hands together before game.
25. Large decks can get iffy (multiple grubs plus roach), see limited "game mode as standard" idea.
26. What to do when a player has discarded all of their cards?
...
# Gameplay
## Setup
40. Each player must have a shuffled deck of a least 9 cards. All players must have an equal amount of cards before at the start of the game.
41. Each player must be dealt 4 cards from this deck, and place one card down face-up, making it "placed". This does not trigger the "Place" ability. Place the rest of the deck face-down and to one side of the play area.
42. Determine a starting player, and proceed clockwise if required.
## Player turn
Unless otherwise stated:
43. If fewer than 3 (of the current player's) cards are placed, you may place one card and then must use its "place" ability, making it placed. A card cannot be placed if the "place" ability cannot trigger. This can only be done once per turn.
44. You may use the "reactivate" ability of one placed card per turn. This cannot be a card you placed this turn.
45. Discarded cards are placed face-down into a separate pile that may not be drawn from. If you discard another player's card, it must be placed in their discard pile.
46. The turn can proceed in any order.
47. If you start your turn with no cards, you may draw up to two as a free action.
48. You may also discard 1 of your held or placed cards on your turn as a free action.
49. Declare when a psyche or element is used for clarity, these abilities do not have to be used if the player does not wish to use them.
...
50. The cost for a psyche must be paid before its effect takes place.
51. Only one psyche or element can be used per player per turn, so you can use an illusion card once on another player's turn for example.
52. Trade variation:
...
## Resolution
53. The game automatically concludes after five rounds (every player has taken a turn), or when a player has no cards left in their deck or hand. At this stage the player with the most cards in their discard pile wins.
54. A player wins immediately if they cause another to have no placed or held cards.
55. A player wins immediately if they have five cards in their hand and:
...
## Clarification:
56. "Any card" includes opponent's cards unless otherwise stated.
57. Unless otherwise stated, when a player must draw a card it will be the topmost card from their deck (not the discard pile).
58. Effects are resolved in the order they are triggered unless otherwise stated.
59. "You" refers to the player who's controlling the card.
# Card Elements
## Bugs
60. Wasp
